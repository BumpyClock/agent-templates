...
Checks (no Rust toolchain needed):
  ...
  - RULES.md links exactly the set of files in rules/ (no broken links, no orphans)
...
# RULES.md index parity
index = INDEX.read_text(encoding="utf-8")
linked = set(re.findall(r'rules/([a-z0-9-]+\.md)', index))
for tgt in sorted(linked):
    if tgt not in rule_names:
        err(f"RULES.md: links missing file rules/{tgt}")
for name in sorted(rule_names):
    if name not in linked:
        err(f"RULES.md: rule rules/{name} is not listed in the index")
