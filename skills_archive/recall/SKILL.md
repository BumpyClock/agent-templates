...
Your context lives in two records. Your own chat history holds what you did and decided. The shared record holds everything that happened around the same code under other names: the symptoms users keep reporting, the fixes that shipped and got reverted, the errors still firing in prod. Inspect relevant source history, issues, documents, and operational evidence for that shared record. A feature with a long bug tail keeps most of its story there, so don't reconstruct it from your transcripts alone.

Transcripts live at `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl` or in `~/.codex/` `~/.copilot/` `~/.claude/`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message.

1. Classify, then route. One specific prior chat to resume is the `session-pickup` playbook, not this. For a requested preference-to-skill task, use [skill-creator](../../.system/skill-creator/SKILL.md) with evidence from user-authorized conversations. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
4. Inspect the shared record when the topic concerns a named feature, file, subsystem, or defect. Start with relevant history and linked reports. Expand to other available sources when evidence remains incomplete. Report unavailable sources and distinguish observations from inference.
