...
Use the categories relevant to the current task. This catalog is reference material, not a mandatory checklist.
...
Treat this profile as an example for a measured workload, not a default change to every project.
Preserve the repository\'s panic strategy, deployment targets, and build tradeoffs unless the task requires a change.
