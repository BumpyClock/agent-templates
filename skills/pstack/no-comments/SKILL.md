...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The [Fix Root Causes](../../programming/systematic-debugging/guide.md#fix-root-causes) and [Redesign from First Principles](../../programming/references/principles/principle-redesign-from-first-principles.md) references guide intent only: fix real causes, redesign as if requirements always existed, never bolt on symptom guards. Neither authorizes widening the fence nor fixing instances outside it.
