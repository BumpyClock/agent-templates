...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The [principle-fix-root-causes](../poteto-mode/references/principle-fix-root-causes.md) and [principle-redesign-from-first-principles](../poteto-mode/references/principle-redesign-from-first-principles.md) references guide intent only: fix real causes, redesign as if requirements always existed, never bolt on symptom guards. Neither authorizes widening the fence nor fixing instances outside it.
