...
- Set `NOTES_DATA_DIR="$PWD/.audit/notes-verify-$RUN_ID/data"` with a unique `RUN_ID` so concurrent runs do not share state.
- Keep proof artifacts outside that disposable data directory.
