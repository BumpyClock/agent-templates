...
Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design changes ownership, inspect relevant history and documented decisions for constraints.

Skip Phase A only when the work is genuinely greenfield with no surrounding system to integrate.

## Phase B: Sketch

Run the **arena** skill with the design-sketch task and the Phase A grounding artifacts. Pass `references/subagent-prompt.md` as each subagent's prompt. Each candidate produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it.
