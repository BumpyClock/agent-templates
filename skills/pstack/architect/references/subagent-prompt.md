...
The orchestrator passes this file through to every parallel candidate subagent during Phase B and fills in the variable inputs around it: the task, the Phase A grounding artifacts, the isolated working directory, and the path to write outputs. The working directory is a git worktree when available, otherwise a per-subagent subdirectory under the sketch dir; what matters is independence between candidates.
