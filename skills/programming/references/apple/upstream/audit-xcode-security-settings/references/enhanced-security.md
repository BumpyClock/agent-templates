...
The cascaded build settings also affect library/framework targets: pointer authentication, security warnings, typed allocators, and C++ hardening.
Use [library distribution guidance](universal-binaries-for-libraries.md) for architecture selection and artifact checks rather than adding a separate `ARCHS` policy here.

## Part A — Build Settings

For capability adoption, assess these settings and any explicit exceptions:

- `ENABLE_ENHANCED_SECURITY = YES` — listed in the capability's `requiredValues`. Cascades to pointer authentication, stack zero init, security compiler warnings, typed allocators, and C++ stdlib hardening unless an explicit override changes the inherited value.
...
1. For authorized project-wide enablement, set `ENABLE_ENHANCED_SECURITY = YES` through the existing project-level configuration. Use the [parent guide](../guide.md#apply-authorized-changes) for tool limitations and verification of effective inheritance. Do not infer project-wide scope from a request concerning one target.
