...
if (-not $winapp) {
    Write-Error "WinApp CLI 0.6 or later is required. Follow references/winui-setup/guide.md, then retry."
    exit 1
}
...
if (-not $installedWinAppVersion) {
    Write-Error "Could not determine the WinApp CLI version. Follow references/winui-setup/guide.md, then retry."
    exit 1
}
if ($installedWinAppVersion -lt $minimumWinAppVersion) {
    Write-Error "WinApp CLI $minimumWinAppVersion or later is required; found $installedWinAppVersion. Follow references/winui-setup/guide.md to upgrade."
    exit 1
}
