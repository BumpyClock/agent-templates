...
> These tables describe the upstream plugin layout. Its adapter sources are not included
> here. This repository uses [its own linker](../scripts/link-agent-templates/link-agent-templates.ts).
> Check current harness documentation for version-specific behavior.
...
| Skill body hard cap | none | No fixed 8 KB cap documented; see below | none | none | none |
...
The [OpenAI skills guide](https://learn.chatgpt.com/docs/build-skills), checked on
2026-09-12, distinguishes the initial skill catalog from the selected skill body.
The catalog includes names, descriptions, and paths. It uses at most 2% of the
model's context window, or 8,000 characters when the context window is unknown.
Codex shortens descriptions first and may omit skills with a warning when the
catalog is too large. It still reads the full `SKILL.md` when selecting a skill.

This is a catalog budget, not an 8 KB skill-body limit. Use conditional references
to disclose task-specific material, not to satisfy an assumed universal byte cap.
Check the installed host's behavior when it differs from the current documentation.
...
- **Codex** — `npx codex-marketplace add wshobson/agents` (or it's auto-discovered as a project
  ...
  straight from `plugins/<name>/skills/`; the selected skill body is read in full.
  Initial catalog budgeting is described above.
